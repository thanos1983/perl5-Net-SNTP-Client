# This Makefile is for the Net::SNTP::Client extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/Net/SNTP/Client.pm]
...
MAKE = make
FULLEXT = Net/SNTP/Client
BASEEXT = Client
PARENT_NAME = Net::SNTP
DLBASE = $(BASEEXT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Net/SNTP/Client.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Net/SNTP
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Net/SNTP
...
TO_INST_PM = lib/Net/SNTP/Client.pm

PM_TO_BLIB = lib/Net/SNTP/Client.pm \
...
DISTVNAME = Net-SNTP-Client-0.19
...
manifypods : pure_all  \
	lib/Net/SNTP/Client.pm
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) \
	  lib/Net/SNTP/Client.pm $(INST_MAN3DIR)/Net::SNTP::Client.$(MAN3EXT) 
...
clean :: clean_subdirs
	- $(RM_F) \
	  *perl.core *$(OBJ_EXT) \
	  ...
	  core.[0-9][0-9][0-9][0-9][0-9] MYMETA.json \
	  lib$(BASEEXT).def core.[0-9][0-9][0-9] \
	  *$(LIB_EXT) $(BASEEXT).x \
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name: Net-SNTP-Client' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   "abstract" : "Perl module to calculate the roundtrip delay d and",' >> META_new.json
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Perl module to calculate the roundtrip delay d and</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Athanasios Garyfalos &lt;garyfalos@cpan.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <PERLCORE VERSION="5,006,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Test::Deep" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/Net/SNTP/Client.pm blib/lib/Net/SNTP/Client.pm 
